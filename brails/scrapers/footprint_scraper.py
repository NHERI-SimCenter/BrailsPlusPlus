...
class FootprintScraper(ABC):
   ...
   def _polygon_area(self, lats,lons,length_unit):
      ...
      # Area in sqm:
      if length_unit=='m': areaout = areaout/(3.28084**2)
      
      return areaout
   
   def _create_asset_inventory(self, footprints :list, attributes :dict, length_unit :str) -> AssetInventory:
   
      """
      This method will be used by the subclasses to form the AssetInventory given footprints & atttributes
      
      Args:
            footprints (list): footprint of each building, each entry a list of lon, lat points
            attributes (dict): dict of atttributes, each value a list of attributes for each bldg
            length_unit (str): indicating length unit 'ft' for example
      
      Returns:
            AssetInventory: A building inventory for buildings in the region.
      ...
      for fp in footprints:
         ...
         attributes['fpAreas'].append(int(self._polygon_area(lats,lons, length_unit)))
